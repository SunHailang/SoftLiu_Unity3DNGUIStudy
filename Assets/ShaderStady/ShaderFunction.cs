...
unity_DeltaTime	float4	Delta time: (dt, 1/dt, smoothDt, 1/smoothDt).
    ...
    纹理映射函数(Texture Map Functions)；（作为单独的一章进行讲解）
    偏导数函数(Derivative Functions)；
    调试函数(Debugging Function)；
8.4.1 数学函数（Mathematical Functions）
三角函数、幂函数、园函数、向量和矩阵的操作函数。这些函数都被重载，以支持标量数据和不同长度的向量作为输入参数。
...
lerp(a, b, f)	        计算(1 - f) * a + b * f 或者 a + f * (b - a) 的 值。即在下限 a 和上限 b 之间进行插值，f 表示权值。
                        注意，如果 a 和 b 是向量，则权值 f 必须是标量或者等长的向量。
lit(NdotL, NdotH, m)	N 表示法向量；L 表示入射光向量；H 表示半角向量；
                        m 表示高光系数。 函数计算环境光、散射光、镜面光的贡献，返回的 4 元向量：X 位表示环境光的贡献，总是 1.0。 
                        Y 位代表散射光的贡献，如果 N • L < 0 ，则为 0；否则为 N • L。
                                Z 位代表镜面光的贡献，如果 N • L < 0 或者 N • H < 0，则为 0；否则为 (N • H) ^ m。 W 位始终为 1.0。
...
sincos(float x, out s, out c)	该函数是同时计算 x 的 sin 值和 cos 值，其中 s=sin(x)，c=cos(x)。
                                该函数用于“同时需要计算 sin 值和 cos 值的情况”，比分别运算要快很多!
...
几何函数，如表 5 所示，用于执行和解析几何相关的计算，例如根据入射光向量和顶点法向量，求取反射光和折射光方向向量。
Cg 语言标准函数库中有 3 个几何函数会经常被使用到，分别是：normalize 函数，对向量进行归一化；
reflect 函数，计算反射光方向向量；refract 函数，计算折射光方向向量。大声呐喊，并要求强烈注意：

着色程序中的向量好进行归一化之后再使用，否则会出现难以预料的错误；
reflect 函数和 refract 函数都存在以“入射光方向向量”作为输入参数，
...
normalize( v)	    归一化向量
reflect(I, N)	    根据入射光方向向量 I，和顶点法向量 N，计算反射光方向向量。其中 I 和 N 必须被归一化，需要非常注意的是，
                    这个 I 是指向顶点的；函数只对三元向量有效。
refract(I,N,eta)	计算折射向量，I 为入射光线，N 为法向量，eta 为折射系数；其中 I 和 N 必须被归一化，如果 I 和 N 之间的夹角太大，
                    则返回（0，0，0），也就是没有折射光线；I 是指向顶点的；函数只对三元向量有效。


8.4.3 纹理映射函数（Texture Map Functions）
下提供 Cg 标准函数库中的纹理映射函数。这些函数被 ps_2_0、ps_2_x、 arbfp1、fp30 和 fp40 等 profiles 完全支持（fully supported）。
所有的这些函数返回四元向量值。
...
所以 ddx 求的是 X 方向上，相邻两个像素的某属性值的变化量；ddy 球的是 Y 方向上，相邻两个像素的某属性值的变化量。

正是由于 ddx 和 ddy 指令是作用于像素级的，所以 ddx 和 ddy 函数只被片段程序所支持。

当在纹理查询函数中使用 ddx 和 ddy 是可以进行图形过滤。所谓图像过滤，
一个比较简单的定义是：对于给定的输入图像 A，要创建新的图像 B，把源图像 A 变换到目标图像 B 的操作就是图像滤波。
一般的变换是调整图像大小、锐化、变化颜色，以及模糊图像等。复杂的过滤有赖于知道究竟需要过滤多少纹理。 
像 GeForce 那样的现代 GPU 提供的偏导函数可以帮助我们。

8.4.5 调试函数（Debugging Function）
函数	功能
void debug(float4 x)	如果在编译时设置了 DEBUG，片段着 色程序中调用该函数可以将值 x 作为 COLOR 语义的终输出；否则该函数什么也不做。
首先顶点 profiles 不支持调试函数。
